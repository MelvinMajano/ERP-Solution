...
class AuthService extends BaseService
{
  ...
  public function checkEmail(string $email): array
  {
    ...
    $preAuthToken = $this->generatePreAuthToken($firstUser->id, $email, $tenants);
   ...
   */
  public function loginPassword(int $userId, int $tenantId, string $password): array
  {
    ...
    // Emitir Session Token (Válido por 8 horas)
  ...
  private function generatePreAuthToken(int $userId, string $email, array $tenants): string
  {
    ...
    $payload = [
      'iat'     => $issuedAt,
      'exp'     => $issuedAt + 600, // 10 minutos de vigencia
      'sub'     => $userId,
      'email'   => $email,
      'type'    => 'pre_auth_token',
      'tenants' => $tenants,
